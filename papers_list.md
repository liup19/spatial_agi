...
- **PDF链接**: https://arxiv.org/pdf/2604.02330
- **HTML链接**: https://arxiv.org/html/2604.02330v1
- **文档**: papers/2026-04-06_01_2604_02330.md
- **文档行数**: ~1,900行
- **分析方法**: GLM WebReader（NotebookLM不可用）
- **核心贡献**: ActionParty多智能体世界模型，主题状态令牌+空间偏向机制解决动作绑定问题，首个同时控制7个玩家的视频世界模型，生成式游戏引擎架构（状态更新-渲染分离），注意力掩码（M_CA + M_SA）实现精确控制
- **与Spatial AGI关系**: 多智能体空间理解和控制，动作与空间推理的解耦，身份一致性保持，状态更新-渲染解耦提高可控性和可解释性，为Spatial AGI提供多智能体系统参考架构
- **应用场景**: 生成视频游戏、虚拟环境模拟、多智能体协同、交互式AI系统、多机器人协作、AR/VR虚拟环境、自动驾驶车队、无人机编队、多智能体强化学习、智能体仿真

---

### 2. Generative World Renderer: Learning to Generate and Invert Real-World Scenes - arXiv:2604.02329
- **相关性**: ⭐⭐⭐⭐⭐ 直接相关
- **关键词**: 生成世界渲染器, 双向渲染, G-buffer, 大规模动态数据集, VLM评估协议, AAA游戏数据, 运动模糊合成, 非侵入式数据收集
- **arXiv链接**: https://arxiv.org/abs/2604.02329
- **PDF链接**: https://arxiv.org/pdf/2604.02329
- **HTML链接**: https://arxiv.org/html/2604.02329v1
- **文档**: papers/2026-04-06_02_2604_02329.md
- **文档行数**: ~1,200行
- **分析方法**: GLM WebReader（NotebookLM不可用）
- **核心贡献**: 从AAA游戏中提取400万连续帧（720p/30 FPS）的同步RGB和五个G-buffer通道，双向渲染（逆+正向）统一在一个框架，VLM评估协议（无GT评估，与人类专家强相关），非侵入式数据收集（ReShade拦截），运动模糊合成显著提升真实世界泛化
- **与Spatial AGI关系**: 大规模动态场景数据集提供高质量训练和测试数据，双向渲染能力统一感知-推理和生成，显式结构化空间表示（G-buffer）支持可解释性和可控性，游戏引擎作为真实世界模拟器的潜力，VLM评估协议开辟无GT场景下的评估新范式
- **应用场景**: 游戏开发、AR/VR应用、仿真训练、视觉特效、机器人导航避障、SLAM、智能监控、自动驾驶、3D重建和数字孪生

---

### 3. Steerable Visual Representations - arXiv:2604.02327
- **相关性**: ⭐⭐⭐⭐ 高相关
- **关键词**: 可引导视觉表示, 早期文本注入, 交叉注意力, 零样本泛化, 异常检测, 对象判别, ViT, 门控机制, 早期vs后期融合
- **arXiv链接**: https://arxiv.org/abs/2604.02327
- **PDF链接**: https://arxiv.org/pdf/2604.02327
- **HTML链接**: https://arxiv.org/html/2604.02327v1
- **文档**: papers/2026-04-06_03_2604_02327.md
- **文档行数**: ~550行
- **分析方法**: GLM WebReader（NotebookLM不可用）
- **核心贡献**: 可引导视觉表示，通过轻量级门控交叉注意力机制（仅21M可训练参数）实现视觉特征的可引导性，早期文本注入（在ViT内部）优于后期融合（在编码后），零样本泛化能力（CORE条件检索96.0%准确率），tanh门控机制控制融合强度（ω=0.6最优），在异常检测、个性化对象区分等任务上SOTA性能
- **与Spatial AGI关系**: 语言作为可编程接口（根据任务动态调整行为，无需重训练），早期融合是细粒度控制的必需设计原则（个性化对象区分上+21.5个百分点），多层次空间表示（从粗粒度场景到细粒度实例），参数高效适配（仅21M可训练参数），更接近人类视觉的自上而下、并行处理机制
- **应用场景**: 机器人（对象识别定位、场景理解导航、操作交互）、AR/VR（选择性渲染、空间标注、交互式探索）、智能监控（特定事件检测、多目标跟踪）、自动驾驶（障碍物识别、语义分割、决策支持）

---

### 4. Modulate-and-Map: Crossmodal Feature Mapping with Cross-View Modulation for 3D Anomaly Detection - arXiv:2604.02328
- **相关性**: ⭐⭐⭐⭐⭐ 直接相关
- **关键词**: 3D异常检测, 跨视图特征调制, 跨模态映射, ModMap框架, DINO-Depth, 双重聚合策略, 特征级仿射变换, 一对多映射歧义
- **arXiv链接**: https://arxiv.org/abs/2604.02328
- **PDF链接**: https://arxiv.org/pdf/2604.02328
- **HTML链接**: https://arxiv.org/html/2604.02328v1
- **文档**: papers/2026-04-06_04_2604_02328.md
- **文档行数**: ~1,563行
- **分析方法**: GLM WebReader（NotebookLM不可用）
- **核心贡献**: ModMap框架（6个组件：图像/深度特征提取器、2个调制器、2个映射网络），跨视图特征调制机制（特征级仿射变换γ ⊙ F + β）解决一对多映射歧义，双重聚合策略（最小值集成精确 + 最大值聚合召回），DINO-Depth专门深度编码器显著优于通用DINO-v2（+22.9%检测/+5.1%分割），在SiM3D数据集上SOTA性能（I-AUROC 0.844, V-AUPRO@1% 0.804）
- **与Spatial AGI关系**: 专门的空间表示学习（DINO-Depth > DINO-v2），特征级调制机制显式建模视图条件，双重聚合策略平衡精确性和召回率，多视图一致性实现鲁棒的3D空间理解，为Spatial AGI的鲁棒性保障层提供技术基础
- **应用场景**: 工业机器人（缺陷检测）、AR/VR（空间标注）、自动驾驶（障碍物识别）、机器人探索（未知环境理解）、智能家居（异常行为检测）、质量控制、安防监控

---

### 5. EventHub: Learning Event Camera Representations via Neural Rendering and Cross-Modal Distillation - arXiv:2604.02331
- **相关性**: ⭐⭐⭐⭐⭐ 直接相关
- **关键词**: 事件相机, 数据工厂, 神经渲染, 跨模态蒸馏, EventHub, SVRaster, 运动自适应渲染, 双向知识迁移, 运动模糊合成
- **arXiv链接**: https://arxiv.org/abs/2604.02331
- **PDF链接**: https://arxiv.org/pdf/2604.02331
- **HTML链接**: https://arxiv.org/html/2604.02331v1
- **文档**: papers/2026-04-06_05_2604_02331.md
- **文档行数**: ~1,218行
- **分析方法**: GLM WebReader（NotebookLM不可用）
- **核心贡献**: 数据工厂范式（神经渲染 + 跨模态蒸馏），两条路径：NVS合成（SVRaster + ESIM）+ 跨模态蒸馏（FoundationStereo → Event），运动自适应渲染机制，双向知识迁移（Event→RGB证明互补性），合成数据质量超越LiDAR监督（M3ED MAE 1.25 vs 2.45，提升50%+），在所有跨域测试（M3ED、MVSEC）中排名第一
- **与Spatial AGI关系**: 数据工厂范式解决事件相机数据稀缺问题，跨模态知识迁移展示Foundation模型的可复用性，事件相机独特价值（高速动态场景、低光照、低功耗），双向知识迁移证明模态互补性，为Spatial AGI的多模态感知层提供数据基础和架构参考
- **应用场景**: 机器人导航（高速避障）、AR/VR（低延迟交互）、智能监控（夜间检测）、工业检测（高速缺陷识别）、无人机（快速响应）、自动驾驶（夜间辅助）

---

**分析方法说明**: 由于 NotebookLM 认证失败，以上 5 篇论文全部使用 GLM WebReader 进行分析。每篇论文文档约 550-1,900 行，深度分析论文的核心方法、实验结果、与 Spatial AGI 的关系及应用场景。

**总文档行数**: ~6,000行（平均1,200行/篇）

**分析方法**: 全部使用GLM WebReader（NotebookLM不可用）

**执行时间**: 约35分钟（5个Subagent并行执行）

**相关性分布**: 4篇直接相关（80%），1篇高相关（20%）

**核心发现**:
1. 多智能体空间理解与控制（ActionParty）
2. 大规模动态场景数据集与双向渲染（Generative World Renderer）
3. 可引导视觉表示与早期融合策略（Steerable Visual Representations）
4. 3D异常检测与跨视图调制（Modulate-and-Map）
